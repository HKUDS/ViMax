"""Regression tests for crash bugs in str/preview helpers and novel2movie __call__."""
...
class TestNovel2MoviePipelineCall(unittest.IsolatedAsyncioTestCase):
    async def test_call_delegates_to_plan_and_render(self):
        pipeline = object.__new__(Novel2MoviePipeline)
        pipeline.plan_text_artifacts = AsyncMock(return_value={"events": 2})
        pipeline.render_video_artifacts = AsyncMock(return_value={"scene_count": 3})

        result = await pipeline(novel_text="Once upon a time.", style="noir")

        pipeline.plan_text_artifacts.assert_awaited_once_with("Once upon a time.")
        pipeline.render_video_artifacts.assert_awaited_once_with(style="noir")
